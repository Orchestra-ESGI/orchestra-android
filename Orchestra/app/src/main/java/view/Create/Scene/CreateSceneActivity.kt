...
class CreateSceneActivity : AppCompatActivity(), OnActionClicked, OnActionLongClicked, AdapterView.OnItemSelectedListener {
    ...
    private val triggerableDeviceTypeList = listOf(
            ...
            DeviceType.contact,
            DeviceType.programmableswitch,
            DeviceType.temperatureandhumidity
    )
    ...
    private fun setUpRoomSpinner() {
        ...
        if (triggerDeviceList.isNotEmpty()) {
            ...
            if (deviceSelected?.type == DeviceType.temperatureandhumidity) {
            ...
            val triggerActionList = when (deviceSelected?.type) {
                DeviceType.programmableswitch -> actionListForSwitch
                else -> actionListForSensor
            }

            val triggerActionTypeList = when (deviceSelected?.type) {
                DeviceType.temperatureandhumidity -> actionTypeTemperatureAndHumidityList
                else -> listOf("state")
            }
 ...
 */
    ...
    private fun retrieveDataAutomation() : Automation {
        ...
        val typeTemperatureOrHumidity = if (deviceSelected.type == DeviceType.temperatureandhumidity) DeviceType.valueOf(triggerActionTypeSpinner.selectedItem as String) else null
